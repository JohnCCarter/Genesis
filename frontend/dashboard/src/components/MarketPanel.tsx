            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                ...
                <label>
                    <input
                        type="checkbox"
                        checked={prob}
                        onChange={(e) => setProb(e.target.checked)}
                    />{' '}
                    Visa prob
                </label>
            ...
            <details style={{ marginBottom: 12 }}>
                    ...
                    <label>
                        <input
                            ...
                            onChange={async (e) => {
                                ...
                                try {
                                    await post(`/api/v2/strategy/auto`, {
                                        AUTO_REGIME_ENABLED: v,
                                    });
                        ...
                        onClick={async () => {
                            try {
                                const result = await post(
                                    `/api/v2/strategy/update-from-regime`
                                );
                ...
                {(auto?.AUTO_REGIME_ENABLED || auto?.AUTO_WEIGHTS_ENABLED) && (
                    ...
                    >
                        ...
                        <br />
                        Regim: {auto?.AUTO_REGIME_ENABLED ? 'Aktiv' : 'Inaktiv'}
                        <br />
                        Vikter: {auto?.AUTO_WEIGHTS_ENABLED ? 'Aktiva' : 'Inaktiva'}
                        <br />
                        {currentRegime && (
                            <span>
                                Aktuell regim:{' '}
                                <strong>{currentRegime.regime || currentRegime}</strong>
                                <br />
                            </span>
                        )}
                        <span>
                            ADX: {currentRegime?.adx_value?.toFixed(1) || 'N/A'}
                            <br />
                        </span>
                        <span>
                            EMA Z: {currentRegime?.ema_z_value?.toFixed(2) || 'N/A'}
                            <br />
                        </span>
                        Aktuella vikter: EMA={settings?.ema_weight?.toFixed(2) || '0.00'},
                        RSI={settings?.rsi_weight?.toFixed(2) || '0.00'}, ATR=
                        {settings?.atr_weight?.toFixed(2) || '0.00'}
                        <br />
                        Vikt-summa: {weightSum.toFixed(2)} (ska vara ~1.0)
                        <br />
                        <small>Settings timestamp: {new Date().toLocaleTimeString()}</small>
                        <br />
                    ...
                    <div>
                        ...
                        <input
                            ...
                            onChange={(e) =>
                                setSettings((s: any) => ({
                                    ...(s || {}),
                                    ema_period: Number(e.target.value),
                                }))
                            }
                        />
                    </div>
                    <div>
                        ...
                        <input
                            ...
                            onChange={(e) =>
                                setSettings((s: any) => ({
                                    ...(s || {}),
                                    rsi_period: Number(e.target.value),
                                }))
                            }
                        />
                    </div>
                    <div>
                        ...
                        <input
                            ...
                            onChange={(e) =>
                                setSettings((s: any) => ({
                                    ...(s || {}),
                                    atr_period: Number(e.target.value),
                                }))
                            }
                        />
                    </div>
                    <div>
                        ...
                        <input
                            ...
                            onChange={(e) =>
                                setSettings((s: any) => ({
                                    ...(s || {}),
                                    ema_weight: Number(e.target.value),
                                }))
                            }
                        />
                    </div>
                    <div>
                        ...
                        <input
                            ...
                            onChange={(e) =>
                                setSettings((s: any) => ({
                                    ...(s || {}),
                                    rsi_weight: Number(e.target.value),
                                }))
                            }
                        />
                    </div>
                    <div>
                        ...
                        <input
                            ...
                            onChange={(e) =>
                                setSettings((s: any) => ({
                                    ...(s || {}),
                                    atr_weight: Number(e.target.value),
                                }))
                            }
                        />
                    </div>
                ...
                >
                    <span style={{ opacity: 0.8 }}>
                        Sum(weights): {weightSum.toFixed(2)}
                    </span>
                    <button
                        type="button"
                        onClick={async () => {
                    ...
                    <button
                        ...
                        onClick={async () => {
                            ...
                            try {
                                ...
                                console.log(
                                    'Backend periods - EMA:',
                                    backendSettings?.ema_period,
                                    'RSI:',
                                    backendSettings?.rsi_period,
                                    'ATR:',
                                    backendSettings?.atr_period
                                );
                            } catch (e) {
                            ...
                            try {
                                ...
                                if (allRegimes?.regimes) {
                                    ...
                                    allRegimes.regimes.forEach((regime: any) => {
                                        console.log(
                                            `${regime.symbol}: ${regime.regime} (ADX: ${regime.adx_value?.toFixed(1) || 'N/A'
                                            }, EMA Z: ${regime.ema_z_value?.toFixed(2) || 'N/A'})`
                                        );
                                    });

                                    // Gruppera efter regim
                                    const trendCount = allRegimes.regimes.filter(
                                        (r: any) => r.regime === 'trend'
                                    ).length;
                                    const balancedCount = allRegimes.regimes.filter(
                                        (r: any) => r.regime === 'balanced'
                                    ).length;
                                    const rangeCount = allRegimes.regimes.filter(
                                        (r: any) => r.regime === 'range'
                                    ).length;

                                    console.log(
                                        `📈 SAMMANFATTNING: Trend: ${trendCount}, Balanced: ${balancedCount}, Range: ${rangeCount}`
                                    );
                                }
                            } catch (e) {
                    ...
                    <button
                        ...
                        onClick={async () => {
                            try {
                                ...
                                console.log(
                                    'Sparade periods - EMA:',
                                    savedData.ema_period,
                                    'RSI:',
                                    savedData.rsi_period,
                                    'ATR:',
                                    savedData.atr_period
                                );
                                console.log(
                                    'Sparade weights - EMA:',
                                    savedData.ema_weight,
                                    'RSI:',
                                    savedData.rsi_weight,
                                    'ATR:',
                                    savedData.atr_weight
                                );
                                setSaveMsg('Sparat');
                ...
                <details style={{ marginTop: 12 }}>
                    ...
                    >
                            ...
                            {allRegimes.map((regime: any, i: number) => (
                                ...
                                >
                                    ...
                                    <span
                                        style={{
                                            color:
                                                regime.confidence_score > 70
                                                    ? '#28a745'
                                                    : regime.confidence_score > 50
                                                        ? '#f6a434'
                                                        : '#d73a49',
                                            fontWeight: 'bold',
                                        }}
                                    ...
                                    <span
                                        style={{
                                            color:
                                                regime.trading_probability > 60
                                                    ? '#28a745'
                                                    : regime.trading_probability > 40
                                                        ? '#f6a434'
                                                        : '#d73a49',
                                            fontWeight: 'bold',
                                        }}
                                    >
                                        {regime.trading_probability}%
                                    </span>
                                    <br />
                                    {/* Recommendation */}
                                    Rekommendation:{' '}
                                    <span
                                        style={{
                                            color:
                                                regime.recommendation === 'STRONG_BUY'
                                                    ? '#28a745'
                                                    : regime.recommendation === 'BUY'
                                                        ? '#2ea043'
                                                        : regime.recommendation === 'WEAK_BUY'
                                                            ? '#f6a434'
                                                            : regime.recommendation === 'HOLD'
                                                                ? '#6f42c1'
                                                                : regime.recommendation === 'AVOID'
                                                                    ? '#d73a49'
                                                                    : '#6a737d',
                                            fontWeight: 'bold',
                                        }}
                                    ...
                                    <small style={{ color: '#6a737d' }}>
                                        ADX: {regime.adx_value?.toFixed(1) || 'N/A'} | EMA Z:{' '}
                                        {regime.ema_z_value?.toFixed(2) || 'N/A'} | Pris:{' '}
                                        {regime.last_close?.toFixed(2) || 'N/A'}
                                    </small>
                                </div>
                            ))}
                        </div>
                        ...
                        >
                            ...
                            >
                                <div>
                                    <strong>Regimer:</strong>
                                    <br />
                                    Trend:{' '}
                                    {allRegimes.filter((r: any) => r.regime === 'trend').length}
                                    <br />
                                    Balanced:{' '}
                                    {
                                        allRegimes.filter((r: any) => r.regime === 'balanced')
                                            .length
                                    }
                                    <br />
                                    Range:{' '}
                                    {allRegimes.filter((r: any) => r.regime === 'range').length}
                                </div>
                                <div>
                                    <strong>Genomsnitt:</strong>
                                    <br />
                                    Säkerhet:{' '}
                                    {(
                                        allRegimes.reduce(
                                ...
                                <div>
                                    ...
                                    Håll:{' '}
                                    {
                                        allRegimes.filter((r: any) => r.recommendation === 'HOLD')
                                            .length
                                    }
                                    <br />
                                    Undvik:{' '}
                                    {
                                        allRegimes.filter((r: any) => r.recommendation === 'AVOID')
                                            .length
                                    }
                                </div>
                            </div>
