    ...
    try:
        ...
        if api_type.lower() == "rest":
            ...
            if abs(expected_digits - actual_digits) > 1:
                return (
                    False,
                    f"REST nonce bör ha ca {expected_digits} siffror (mikrosekunder), men har {actual_digits}",
                )
            ...
            if abs(expected_digits - actual_digits) > 1:
                return (
                    False,
                    f"WebSocket nonce bör ha ca {expected_digits} siffror (millisekunder), men har {actual_digits}",
                )
