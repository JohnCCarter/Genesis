...
class TestRedisIntegration:
    ...
    def test_redis_pub_sub(self):
        ...
        message = json.dumps({"type": "config_update", "key": "DRY_RUN_ENABLED", "generation": 5})
...
class TestClusterConsistency:
    ...
    def test_cross_node_cache_invalidation(self):
        ...
        # Node 1 sätter värde
        shared_redis.hset(
            "config:values", "TEST_KEY", json.dumps({"key": "TEST_KEY", "value": "value1", "generation": 1})
        )

        # Node 2 uppdaterar värde
        shared_redis.hset(
            "config:values", "TEST_KEY", json.dumps({"key": "TEST_KEY", "value": "value2", "generation": 2})
        )

        # Publicera invalidation
        invalidation_message = json.dumps({"type": "config_update", "key": "TEST_KEY", "generation": 2})
    ...
    def test_concurrent_updates_consistency(self):
        ...
        def update_worker(worker_id):
            ...
            time.sleep(0.01)
            result = {"worker": worker_id, "generation": worker_id + 10, "timestamp": time.time()}
...
class TestAtomicOperations:
    ...
    def test_atomic_batch_update(self):
        ...
        for key, value in updates.items():
            pipe.hset("config:values", key, json.dumps({"key": key, "value": value, "generation": 1}))
    ...
    def test_compare_and_set_atomic(self):
        ...
        # Sätt initial värde
        self.mock_redis.hset(
            "config:values", "TEST_KEY", json.dumps({"key": "TEST_KEY", "value": "initial", "generation": 1})
        )
        ...
        def atomic_compare_and_set(key, expected_value, new_value, expected_generation):
            ...
            if current:
                ...
                if current_data["value"] == expected_value and current_data["generation"] == expected_generation:
                    # Uppdatera atomiskt
                    new_data = {"key": key, "value": new_value, "generation": expected_generation + 1}
...
class TestCacheCoherency:
    ...
    def test_cache_warming(self):
        ...
        self.mock_redis.hset(
            "config:values", "WARM_KEY", json.dumps({"key": "WARM_KEY", "value": "warm_value", "generation": 1})
        )
