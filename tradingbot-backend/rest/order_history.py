    ...
    async def get_orders_history(
        ...
        try:
            ...
            try:
                ...
                logger.error(f"JSON parsing fel för orders history: {json_error}")
    ...
    async def get_order_trades(self, order_id: int) -> list[TradeItem]:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"JSON parsing fel för order trades: {json_error}")
                logger.error(f"Response status: {response.status_code}")
                logger.error(f"Response text (first 500 chars): {response.text[:500]}")
                raise RuntimeError(f"JSON parsing failed: {json_error}")
    ...
    async def get_trades_history(
        ...
        try:
            ...
            try:
                ...
                logger.error(f"JSON parsing fel för trades history: {json_error}")
                logger.error(f"Response status: {response.status_code}")
                logger.error(f"Response text (first 500 chars): {response.text[:500]}")
                raise RuntimeError(f"JSON parsing failed: {json_error}")
    ...
    async def get_ledgers(
        ...
        try:
            ...
            try:
                ...
                logger.error(f"JSON parsing fel för ledger: {json_error}")
                logger.error(f"Response status: {response.status_code}")
                logger.error(f"Response text (first 500 chars): {response.text[:500]}")
                raise RuntimeError(f"JSON parsing failed: {json_error}")
