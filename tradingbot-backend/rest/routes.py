...
class WSOrderOnRequest(BaseModel):
    """Ren WS-order (on) requestmodell."""
    ...
    side: str | None = None
    price: str | None = None
    price_trailing: str | None = None
    price_aux_limit: str | None = None
    flags: int | None = None
...
@router.post("/ws/order", response_model=OrderResponse)
async def ws_order_place(payload: WSOrderOnRequest, _: bool = Depends(require_auth)):
    """Lägg order direkt via WS 'on' (utan REST)."""
    try:
        # Dry Run stöd
        try:
            import services.runtime_config as rc
            ...
            if rc.get_bool("DRY_RUN_ENABLED", False):
                return OrderResponse(success=True, data={"dry_run": True, "order": payload.dict()})
        except Exception:
            pass

        from services.bitfinex_websocket import bitfinex_ws

        result = await bitfinex_ws.order_on(payload.dict())
        if not result.get("success"):
            return OrderResponse(success=False, error=str(result.get("error")))
        return OrderResponse(success=True, data=result)
    except Exception as e:
        logger.exception(f"WS order (on) fel: {e}")
        return OrderResponse(success=False, error=str(e))
...
async def get_risk_status(_: bool = Depends(require_auth)):
    ...
    try:
        ...
        risk_overview = {}
        risk_overview["transport_circuit_breaker"] = transport_cb
    except Exception:
        pass
...
async def get_overall_health(_: bool = Depends(require_auth)):
    ...
    try:
        ...
        overall_health = health_watchdog.get_overall_health()
        return status
    except Exception as e:
...
async def update_unified_trading_windows(req: UnifiedWindowsUpdateRequest, _: bool = Depends(require_auth)):
    ...
    try:
        ...
        tw.save_rules(windows=req.windows)
...
async def get_all_regimes(_: bool = Depends(require_auth)):
    ...
    try:
        from services.symbols import SymbolService
        from config.settings import Settings as _S3
