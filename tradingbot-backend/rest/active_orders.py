...
class ActiveOrdersService:
    ...
    async def update_order(self, order_id: int, price: Optional[float] = None, amount: Optional[float] = None) -> Dict[str, Any]:
        """
    ...
    async def cancel_all_orders(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                logger.info(f"🌐 REST API: Avbryter alla ordrar")
                response = await client.post(
                    f"{self.base_url}/{endpoint}",
                    headers=headers
                )
                try:
                    response.raise_for_status()
                    result = response.json()
                    logger.info(f"✅ REST API: Alla ordrar avbrutna framgångsrikt")
                    return {
                        "success": True,
                    ...
                    if e.response is not None and e.response.status_code == 404:
                        logger.warning("⚠️ cancel/all inte tillgänglig (404). Faller tillbaka till att avbryta individuellt.")
                    else:
                        raise
