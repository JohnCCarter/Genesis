...
class OrderValidator:
    ...
    def validate_order(self, order: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        if order_type not in self.order_types:
            return False, f"Ogiltig ordertyp: {order_type}. Giltiga typer: {', '.join(self.order_types.keys())}"
        ...
        for param in required_params:
            if param not in order or order[param] is None:
                return False, f"Saknad parameter: {param} krävs för ordertyp {order_type}"
    ...
    def format_order_for_bitfinex(self, order: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "price" in formatted_order and formatted_order["price"] is not None and not isinstance(formatted_order["price"], str):
