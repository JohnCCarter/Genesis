    ...
    def _get_cached_or_fetch(self, url: str, section: str) -> dict:
        ...
        try:
            ...
            with open(cache_file, "w", encoding="utf-8") as f:
                json.dump(response.json() if "application/json" in response.headers.get("Content-Type", "") 
                          else {"html": response.text}, f, ensure_ascii=False, indent=2)
            
            # Vänta lite för att inte överbelasta servern
            time.sleep(0.5)
            
            return response.json() if "application/json" in response.headers.get("Content-Type", "") else {"html": response.text}
            ...
            try:
                ...
                if title_elem and path_elem:
                    endpoint = {
                        ...
                        "description": elem.select_one(".endpoint-desc").get_text(strip=True) if elem.select_one(".endpoint-desc") else "",
                        "parameters": []
                    }
    ...
    def fetch_order_types(self) -> Dict[str, Dict]:
        ...
        self.order_types = {
            "EXCHANGE LIMIT": {
                ...
                "optional_params": ["price_trailing", "price_aux_limit", "price_oco_stop", "flags"]
            },
            "EXCHANGE MARKET": {
