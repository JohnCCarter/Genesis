...
class BitfinexAuthScraper:
    ...
    def _parse_rest_auth_html(self, html_content: str) -> Dict[str, Any]:
        ...
        rest_auth_header = soup.find(string=lambda text: text and isinstance(text, str) and "REST Authenticated Endpoints" in text)
        ...
        if rest_auth_header and rest_auth_header.parent:
            ...
            for strong in all_strongs:
                ...
                if any(keyword in text for keyword in ["Wallets", "Orders", "Positions", "Margin", "Funding", "Account", "Merchants"]):
                    endpoint_headers.append(strong)
        else:
            # Fallback till att leta efter specifika nyckelord i h3/h4
            endpoint_headers = soup.find_all(["h3", "h4"], string=lambda text: text and any(
                keyword in text for keyword in ["Wallets", "Orders", "Positions", "Margin", "Funding", "Account"]
            ))
    ...
    def _parse_ws_auth_html(self, html_content: str) -> Dict[str, Any]:
        ...
        if not event_headers:
            event_headers = soup.find_all(["h3"], string=lambda text: text and any(
                keyword in text.lower() for keyword in ["list of account", "list of ws"]
            ))
...
"""
    ...
    def compare_with_current_implementation(self, rest_auth_code: str = None, ws_auth_code: str = None) -> Dict[str, Any]:
        """
        ...
        if rest_auth_code:
            ...
            if "timestamp() * 1_000_000" not in rest_auth_code and "mikrosekunder" in recommendations["rest"]["nonce_generation"]:
            ...
            for key, value in recommendations["rest"]["headers"].items():
                if value not in rest_auth_code:
                    ...
                    results["rest"]["differences"].append({
                        "type": "header",
                        "recommendation": f"Använd header: {value}"
                    })
        
        # Jämför WebSocket implementation
        if ws_auth_code:
            # Kontrollera nonce-generering
            if "timestamp() * 1000" not in ws_auth_code and "millisekunder" in recommendations["websocket"]["nonce_generation"]:
