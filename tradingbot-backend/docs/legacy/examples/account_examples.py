...
from rest.wallet import WalletBalance, get_wallets, get_wallet_by_type_and_currency
from rest.positions import Position, get_positions, get_position_by_symbol
...
async def get_wallets_example():
    ...
    try:
        ...
        for wallet in wallets:
            print(f"{wallet.wallet_type.upper()} - {wallet.currency}: {wallet.balance} " +
...
async def get_margin_info_example():
    ...
    try:
        ...
        try:
            ...
            print("OBS: Detta kan bero på att ditt konto inte har margin aktiverat eller att du använder ett testkonto.")
        ...
        try:
            ...
            if orders:
                for order in orders:
                    ...
                    print(f"{created} - {order.symbol} {order.type}: {order.amount} @ {order.price} ({order.status})")
            else:
            ...
            if orders:
                ...
                if trades:
                    for trade in trades:
                        ...
                        print(f"{executed} - {trade.amount} @ {trade.price} (Fee: {trade.fee} {trade.fee_currency})")
                else:
                    print(f"Inga trades hittade för order {first_order.id}")
        except Exception as e:
            print("\n=== Orderhistorik ===")
            print(f"Kunde inte hämta orderhistorik: {e}")
            print("OBS: Detta kan bero på att ditt konto inte har några ordrar eller att du använder ett testkonto.")
    ...
    try:
        try:
            ...
            if trades:
                for trade in trades:
                    executed = trade.executed_at.strftime("%Y-%m-%d %H:%M:%S")
                    print(f"{executed} - {trade.symbol}: {trade.amount} @ {trade.price} (Fee: {trade.fee} {trade.fee_currency})")
            else:
            ...
            print("OBS: Detta kan bero på att ditt konto inte har några trades eller att du använder ett testkonto.")
        ...
        try:
            ...
            if ledgers:
                for ledger in ledgers:
                    ...
                    print(f"{created} - {ledger.wallet_type} {ledger.currency}: {ledger.amount} (Balance: {ledger.balance}) - {ledger.description}")
            else:
            ...
            if usd_ledgers:
                for ledger in usd_ledgers:
                    created = ledger.created_at.strftime("%Y-%m-%d %H:%M:%S")
                    print(f"{created} - {ledger.wallet_type}: {ledger.amount} (Balance: {ledger.balance}) - {ledger.description}")
            else:
                print("Inga USD ledger-poster hittade")
        except Exception as e:
            print("\n=== Ledger ===")
            print(f"Kunde inte hämta ledger-poster: {e}")
            print("OBS: Detta kan bero på att ditt konto inte har några ledger-poster eller att du använder ett testkonto.")
