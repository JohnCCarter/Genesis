...
class DummySIOClient:
    ...
    async def emit(self, event, data):
        ...
        if event == "wallet_update":
            logger.info(f"  Plånboksuppdatering: {data['wallet_type']} {data['currency']} {data['balance']}")
        elif event == "wallet_snapshot":
            logger.info(f"  Plånboks-snapshot: {len(data)} plånböcker")
        elif event == "position_update":
            logger.info(f"  Positionsuppdatering: {data['symbol']} {data['status']} {data['amount']}")
    ...
    try:
        ...
        def on_position_update(positions):
            ...
            for position in positions:
                position_type = "LONG" if position.get('is_long', False) else "SHORT" if position.get('is_short', False) else "NEUTRAL"
                print(f"  {position['symbol']} {position['status']} {position_type}: {position['amount']} @ {position.get('base_price', 'N/A')}")
