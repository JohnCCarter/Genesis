...
def display_rest_auth_info():
    ...
    if "rest" in examples and "python" in examples["rest"] and "build_auth_headers" in examples["rest"]["python"]:
...
def display_ws_auth_info():
    ...
    if "auth_parameters" in ws_info and ws_info["auth_parameters"]:
        ...
        for param in ws_info["auth_parameters"]:
            print(f"  | {param.get('field', 'N/A')} | {param.get('type', 'N/A')} | {param.get('description', 'N/A')} |")
    else:
    ...
    if "examples" in ws_info and ws_info["examples"]:
        for example in ws_info["examples"]:
            print(f"  - [{example.get('title', 'N/A')}]({example.get('link', 'N/A')}) ({example.get('language', 'N/A')})")
    else:
    ...
    if "websocket" in examples and "python" in examples["websocket"] and "build_ws_auth_payload" in examples["websocket"]["python"]:
...
def compare_auth_implementations():
    ...
    # REST skillnader
    rest_diffs = []
    if "mikrosekunder" in recommendations['rest']['nonce_generation'] and "timestamp() * 1_000_000" not in str(build_auth_headers):
    ...
    ws_diffs = []
    if "millisekunder" in recommendations['websocket']['nonce_generation'] and "timestamp() * 1000" not in str(build_ws_auth_payload):
