...
from models.api_models import OrderResponse, OrderType, OrderSide
from rest.active_orders import (
    get_active_orders, get_active_orders_by_symbol, get_active_orders_by_type,
    get_active_orders_by_side, get_order_by_id, get_order_by_client_id,
    update_order, cancel_all_orders, cancel_orders_by_symbol
)
...
async def get_active_orders_example():
    ...
    try:
        try:
            ...
            print("OBS: Detta kan bero på att ditt konto inte har några aktiva ordrar eller att du använder ett testkonto.")
...
async def get_active_orders_by_side_example():
    ...
    try:
        try:
            ...
            if orders:
                for order in orders:
                    print(f"{order.id}: {order.symbol} - {abs(order.amount)} @ {order.price} ({order.status})")
            else:
...
async def place_and_update_order_example():
    ...
    try:
        try:
            ...
            if updated_order:
                print(f"\n=== Uppdaterad Order ===")
                print(f"{updated_order.id}: {updated_order.symbol} - {updated_order.amount} @ {updated_order.price} ({updated_order.status})")
            else:
                print("\nKunde inte hitta den uppdaterade ordern")
        except Exception as e:
            print("\n=== Lägga och Uppdatera Order ===")
            print(f"Kunde inte lägga eller uppdatera order: {e}")
            print("OBS: Detta kan bero på att ditt konto inte har tillräckligt med saldo eller att du använder ett testkonto.")
