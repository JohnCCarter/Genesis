import pytest
pytestmark = pytest.mark.skip(reason="Legacy direct Bitfinex test – skipped in current backend focus")
"""
...
def _build_authentication_headers(endpoint, payload = None):
    import time
    nonce = str(int(time.time() * 1_000_000))  # Mikrosekunder för att säkerställa unikhet
